...
public interface ProjectRepository extends JpaRepository<Project, Long> {
   ...
   // ProjectWithVotesDto getProjectWithVotesDto();
    ...
    @Query("SELECT new com.aniamadej.votingsystem.models.dtos.ProjectWithVotesDto(p.projectName, p.projectDescription, p.active, sum(case when v.voteValue>0 then 1 else 0 end), sum(case when v.voteValue<=0 then 1 else 0 end)) from Project p left join Vote v on p.id=v.project where p.id=?1")
    ProjectWithVotesDto getProjectWithVotesDto(Long projectId);
